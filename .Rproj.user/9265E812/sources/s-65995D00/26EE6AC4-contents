...
#' #' @param sp.parm numeric
#' #' @param return.all logical
#' #' 
#' #' @author JM Eastman
#' #' 
#' #' @return an array
...
#'       stop(
#'         cat(
#'           "Unrecognized method declaration or insufficient information supplied:\n\tIs 'parm' set to a non-null value?\n\n"
#'         )
#'       )
...
#'     }
#'     
#'     o.foo = spacodi.all.nodes(
#'       sp.plot = sp.plot,
#'       phy = phy,
#'       sp.parm = sp.parm,
#'       return.all = ifelse(obs.only == TRUE, TRUE, FALSE)
#'     )
#'     o = o.foo[, sp.parm]
...
#'       orig.match <-
#'         match(as.numeric(o.orig$node.ID), as.numeric(row.names(e.out)))
...
#'           "p.value",
#'           "node.ID",
#'           paste("obs", sp.parm, sep = "."),
#'           paste("m.exp", sp.parm, sep = "."),
#'           "valid.comparisons"
#'         )
#'         row.names(r.test) = r.test$node.ID
#'         if (return.all == TRUE) {
#'           o.orig = as.data.frame(
#'             spacodi.all.nodes(
#'               sp.plot = sp.plot,
#'               phy = phy,
#'               return.all = TRUE,
...
#'         names(Bst.out) = c(
#'           paste("observed", sp.parm, sep = "."),
#'           paste("expected", sp.parm, sep = "."),
#'           "randomization.test"
#'         )
#'         return(Bst.out)
#'       }
...
#'         names(Bst.out) = c(paste("observed", sp.parm, sep = "."),
#'                            paste("expected", sp.parm, sep = "."))
#'         return(Bst.out)
#'       }
#'     }
