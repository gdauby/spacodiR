#####
# spacodi.treeplot <-
#   function(spacodi.permutations,
#            phy,
#            cex = list(pch = 1.5, tip = 0.5, legend = 0.75),
#            transp = 0.8,
#            sig.plot = TRUE,
#            cut.off = 0.05,
#            cols = list("white", "gray", "black"),
#            main = TRUE,
#            outfile = NULL,
#            add.id = FALSE,
#            ...) {
...
#     #	on.exit(par(op))
#     
#     if (class(spacodi.permutations) == "list")
#       base <- as.data.frame(spacodi.permutations[[1]])
#     else
#       base <- as.data.frame(spacodi.permutations)
#     if (nrow(base) != phy$Nnode)
#       stop(
#         "SPACoDi.plot requires data (whether NA not) for every node in the spacodi.permutations."
#       )
#     
...
#       } else {
#         stop(
#           "Cannot perform significance plot without there being a randomization.test within the spacodi.permutations"
#         )
#       }
#     }
#     
...
#         } else {
#           col.i <-
#             1 + (bst.i - min(bst.i, na.rm = TRUE)) * 99 / (max(bst.i, na.rm = TRUE) -
#                                                              min(bst.i, na.rm = TRUE))
#           col.i <- round(col.i)
...
#         if (sig.plot) {
#           nodelabels(
#             bg = add.transparency(col.i, transp),
#             col = "gray",
#             frame = "circle",
#             pch = 21,
#             cex = cex$pch
...
#             "bottomleft",
#             legend = c(
#               paste(sp.parm, ": as expected", sep = ""),
#               paste(sp.parm, ": larger than expected", sep = ""),
#               paste(sp.parm, ": smaller than expected", sep = "")
#             ),
#             fill = add.transparency(c(NA, cols[[2]], cols[[3]]), transp),
#             bg = add.transparency("white", 0.5),
#             box.lty = "blank",
#             border = "gray",
#             cex = cex$legend
#           )
...
#           legend(
#             "bottomleft",
#             legend = sprintf("%6.2f", rev(seq(
#               min(bst.i, na.rm = TRUE),
#               max(bst.i, na.rm = TRUE),
#               length = 9
#             ))),
#             fill = add.transparency(rev(diverge_hcl(9)), transp),
#             bg = add.transparency("white", 0.5),
#             box.lty = "blank",
#             border = "gray",
#             cex = cex$legend
...
#       if (main)
#         title(
#           main = paste("Values of ", sp.parm, sep = ""),
#           outer = TRUE,
#           line = -1
#         )
#       if (!is.null(outfile))
#         dev.off()
#     } else {
#       stop("Cannot interpret spacodi.permutations for plotting.")
#     }
#     #	invisible()
#   }
...
#     if (n ^ 2 > 16)
#       warning("Tip labels are unlikely to plot properly with this many trees.")
#     for (i in 1:l.plot) {
#       tip.labs = vector()
#       for (j in 1:length(tips)) {
#         if (!is.na(match(as.vector(tips[j]), as.vector(plots[[i]]$species))))
#           tip.labs[j] = 1
#         else
#           tip.labs[j] = 0
#       }
...
#       dev.off()
#     if (print.labs) {
#       res = list(plots, phy$tip.label)
#       names(res) = c("groups", "tip.labels")
#       print(res)
#       dev.new()
#       plot(phy, direction = "u")
#     }
#     invisible()
#   }
